...
x_train, x_test, y_train, y_test = train_test_split(value_ss,variable_label,train_size=0.8,test_size=0.2,random_state=42,shuffle=True)
y_train = y_train - 1
y_test = y_test -1
for bandwidth in np.linspace(1.5, 2, 5):
# bandwidth = 0.5
    ...
    model = models.Sequential([
        RBFLayer(units=len(centers), gamma=gamma, center_initializer=centers, input_shape=(36,)),
    ...
    with open('results.txt', 'a') as file:
        file.write('Bandwidth: {:.3f}; '.format(bandwidth))
